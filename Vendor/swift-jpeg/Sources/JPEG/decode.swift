...
/* Modified by TUIkit: known frame heights may not grow or accept extraneous entropy/DNL data. */
...
extension JPEG.Bitstream
{
    ...
    func validatePadding(after position:Int) throws
    {
        guard 0 ... self.count ~= position
        else
        {
            throw JPEG.DecodingError.truncatedEntropyCodedSegment
        }

        let paddingCount:Int = self.count - position
        guard paddingCount < 8,
            (position ..< self.count).allSatisfy(
            {
                self[$0, as: UInt8.self] == 1
            })
        else
        {
            throw JPEG.DecodingError.invalidEntropyCodedSegmentPadding
        }
    }
}
...
extension JPEG.Data.Spectral.Plane
{
        ...
        extend:Bool) throws
    {
        ...
        try bits.validatePadding(after: b)
    }
        ...
        while true
        {
            ...
            case .scan:
                ...
                for index:Int in 0...
                {
                    ...
                    {
                        if first, frame!.size.y > 0, case .height = marker.type
                        {
